...
export function renderTask(number) {
  ...
  return `<header class="page-head"><p class="eyebrow">Задание ${pad(number)} / 07 · 90 минут</p><h1>${esc(item.title)}</h1><p class="lead">${esc(item.goal || item.ability)}</p></header>
  <div class="task-layout">${lessonNav(number)}<div class="task-main">
    ...
    ${renderContext(item)}
    <section class="task-section panel"><p class="eyebrow">Практическая работа</p><h2>Задание</h2><p class="task-section-intro">Галочки хранятся только в этом браузере и не являются сдачей.</p><ol class="steps">${item.steps.map((step, index) => `<li class="${checked.has(index) ? 'checked' : ''}"><input type="checkbox" data-step="${index}" ${checked.has(index) ? 'checked' : ''} aria-label="Шаг ${index + 1}"><label>${inline(step)}</label></li>`).join('')}</ol></section>
    <section class="task-section panel"><p class="eyebrow">Артефакты</p><h2>Что сдавать</h2><div class="deliver-grid"><article class="deliver-card"><span>Основной результат</span><b>${esc(item.artifact)}</b></article>${item.minimum.map(value => `<article class="deliver-card"><span>Обязательно</span><b>${inline(value)}</b></article>`).join('')}</div></section>
    ...
    <section class="progress-action panel"><div><b>${done ? 'Работа отмечена подготовленной' : 'Закончили подготовку?'}</b><p>Реальная сдача идёт через GitVerse и объяснение преподавателю.</p></div><button class="${done ? '' : 'primary'}" id="toggle-done" data-number="${number}">${done ? 'Снять отметку' : 'Отметить подготовленной'}</button></section>
