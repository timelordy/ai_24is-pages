...
function home() {
  ...
  return `<section class="hero"><div><p class="eyebrow">Методы искусственного интеллекта</p><h1>Семь полноценных<br>лабораторных работ</h1><p class="lead">Каждая пара — отдельный 90-минутный эксперимент: объяснение, практика и защита. Со второй пары работаем в OpenCode через бесплатный Groq endpoint.</p><div class="role-choice"><a class="role-card" href="#lesson-1"><b>Я студент</b><span>Открыть первую лабораторную →</span></a><a class="role-card teacher-card" href="#teacher"><b>Я преподаватель</b><span>Открыть сценарий занятия →</span></a></div></div><aside class="next"><p class="eyebrow">${index < 0 ? "Расписание завершено" : "Ближайшее занятие"} · ${escapeHTML(group.group)}</p>${lesson ? `<p class="meta">${dateLabel(group.dates[index])} 2026 · ${group.time}<br>Аудитория ${escapeHTML(group.room)}</p><h2>${pad(index + 1)} / ${escapeHTML(lesson.title)}</h2><p><b>Исследовательский вопрос:</b> ${escapeHTML(depth[index]?.question || lesson.learn)}</p><p><b>Результат:</b> ${escapeHTML(depth[index]?.product || lesson.result)}</p><a class="button" href="#lesson-${index + 1}">Открыть лабораторную →</a>` : '<h2>Все семь встреч позади</h2><p>Материалы остаются доступны. Проверьте комплект отчётов и подготовьтесь к защите.</p><a class="button" href="#work">Моя работа</a>'}</aside></section><section class="panel"><h2>На чём запускаем</h2><p><b>Основной маршрут:</b> личный Groq Free Plan → OpenCode → <code>qwen/qwen3.6-27b</code>. Резервы: <code>openai/gpt-oss-20b</code>, OpenRouter Free, NVIDIA trial и Ollama.</p><p>Общий ключ преподавателя группе не раздаётся. У каждой пары есть предел обращений и шагов.</p><a href="assets/materials/COURSE_RUNBOOK.html">Открыть схему запуска и лимиты →</a></section><section class="principles"><article><b>Не обзор сервисов</b><p>Каждая технология проверяется серией опытов на одном проекте.</p></article><article><b>65–70 минут работы</b><p>Нужно получить повторяемые результаты, ошибки и измерения.</p></article><article><b>Защита по новому входу</b><p>Готовый скриншот не заменяет объяснение и живую проверку.</p></article></section><h2>Семь лабораторных работ</h2><div class="lesson-list">${rows()}</div>`;
}
...
function lessonPage(n, role = "student") {
  ...
  return `<p class="eyebrow">Лабораторная ${pad(n)} / 07 · 90 минут</p><h1>${escapeHTML(c.title)}</h1><div class="notice"><b>Среда:</b> ${escapeHTML(d.runtime)}<br><b>Лимит:</b> ${escapeHTML(d.budget)}</div><div class="lesson-summary"><div><span class="step-label">Что создаём</span><p>${escapeHTML(d.product)}</p></div><div><span class="step-label">Объём сдачи</span><p>${escapeHTML(d.minimum)}</p></div></div><div class="role-tabs"><a class="${role === "student" ? "active" : ""}" href="#student-${n}">Студенту: лабораторная</a><a class="teacher-tab ${role === "teacher" ? "active" : ""}" href="#teach-${n}">Преподавателю: как провести</a></div>${role === "teacher" ? teacherSection : studentSection}<details class="more"><summary>Теория и дополнительное задание</summary><h2>Короткое объяснение</h2><p>${escapeHTML(c.explain)}</p><h2>Для сильных</h2><p>${escapeHTML(c.extension)}</p></details><aside class="download-bar"><div><b>Файлы лабораторной ${n}</b><span>Распакуйте ZIP и сначала откройте LAB.md.</span></div><a class="button" href="assets/downloads/lesson-${pad(n)}.zip" download>Скачать starter ZIP</a><a class="button secondary" href="assets/materials/lessons/${pad(n)}/report-template.md" download>Скачать шаблон отчёта</a></aside><div class="actions lesson-nav">${n > 1 ? `<a href="#${role === "teacher" ? "teach" : "student"}-${n - 1}">← Лабораторная ${n - 1}</a>` : ""}${n < 7 ? `<a href="#${role === "teacher" ? "teach" : "student"}-${n + 1}">Лабораторная ${n + 1} →</a>` : role === "teacher" ? '<a href="#teacher">К списку сценариев →</a>' : '<a href="#work">Проверить свою работу →</a>'}</div>`;
}
